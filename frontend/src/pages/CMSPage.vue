  ...
  methods: {
    ...
    handleMakeACopy() {
      this.showAlert({
        ...
        description:
          "Are you sure you want to make a copy of this Toolbox Talk?",
        actions: [
          {
            label: "Cancel",
          },
          {
            label: "Yes",
            handler: () => this.handleConfirmMakeACopy(),
          },
        ],
      });
    },
    async handleConfirmMakeACopy() {
      try {
        await apiClient.post("/make-toolbox-copy", {
          toolbox_talk_id: this.$route.params.id,
        });
